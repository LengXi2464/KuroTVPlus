...
// 绕过 webpack 静态分析，获取真正�?Node.js require
...
async function getEnabledSourceScriptByKey(key: string) {
  ...
  if (!item) {
    throw new Error('脚本不存�?);
  }
  if (!item.enabled) {
    throw new Error('脚本已停�?);
  }
  return item;
}
...
export async function executeSavedSourceScript(input: {
  ...
  if (typeof hook !== 'function') {
    throw new Error(`脚本未实�?${input.hook} hook`);
  }
...
export async function resolveScriptDetailPlaybacks(input: {
  ...
  // 预检查：编译一次脚本，判断是否实现�?resolvePlayUrl
  ...
  // 已实�?resolvePlayUrl，创建一�?context 复用
