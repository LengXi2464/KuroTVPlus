...
/**
 * 统一�?Redis 适配器接�? * 只抽�?API 命名差异，不处理序列化（由调用者负责）
 */
...
/**
 * 标准 Redis 客户端适配器（用于 Redis �?Kvrocks�? * 只处�?API 命名，不处理序列�? */
...
/**
 * Upstash Redis 客户端适配器（用于 Upstash REST API�? * 处理 API 命名差异�?Upstash 的自动序列化
 */
export class UpstashRedisAdapter implements RedisAdapter {
  ...
  async hSet(key: string, fieldOrData: string | Record<string, string>, value?: string): Promise<number> {
    if (typeof fieldOrData === 'string') {
      // Upstash 会自动序列化，但我们传入的已经是字符串，所以直接存�?      return this.client.hset(key, { [fieldOrData]: value! });
    } else {
      return this.client.hset(key, fieldOrData);
    }
  }
  ...
  async hGet(key: string, field: string): Promise<string | null> {
    ...
    // Upstash 可能返回对象、字符串或其他类�?    // 需要统一转换为字符串
  ...
  async get(key: string): Promise<string | null> {
    ...
    if (val === null || val === undefined) return null;
    // Upstash 可能返回对象、字符串或其他类�?    if (typeof val === 'string') return val;
    if (typeof val === 'object') return JSON.stringify(val);
    return String(val);
  }
