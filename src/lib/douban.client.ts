...
export async function fetchDoubanCategories(
  ...
  if (!['tv', 'movie'].includes(kind)) {
    throw new Error('kind 参数必须�?tv �?movie');
  }

  if (!category || !type) {
    throw new Error('category �?type 参数不能为空');
  }

  if (pageLimit < 1 || pageLimit > 100) {
    throw new Error('pageLimit 必须�?1-100 之间');
  }
...
/**
 * 统一的豆瓣分类数据获取函数，根据代理设置选择使用服务�?API 或客户端代理获取
 */
...
export async function fetchDoubanList(
  ...
  // 验证参数
  if (!tag || !type) {
    throw new Error('tag �?type 参数不能为空');
  }

  if (!['tv', 'movie'].includes(type)) {
    throw new Error('type 参数必须�?tv �?movie');
  }
...
/**
 * 统一的豆瓣详情数据获取函数，根据代理设置选择使用服务�?API 或客户端代理获取
 */
