...
export async function GET(request: NextRequest) {
  try {
    ...
    // 使用小雅的搜索引�?    const searchUrl = `${xiaoyaConfig.ServerURL}/search?box=${encodeURIComponent(keyword)}&type=${type}&url=`;
    ...
    while ((match = linkRegex.exec(html)) !== null) {
      ...
      // 跳过返回首页和频道链�?      if (path === '/' || path.startsWith('http')) {
        continue;
      }
      ...
      // 提取文件名（路径的最后一部分�?      const pathParts = displayText.split('/');
