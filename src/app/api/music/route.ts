...
// 正在下载的音频任务追踪（防止重复下载�?const downloadingTasks = new Map<string, Promise<void>>();
...
async function cacheAudioToOpenList(
  ...
  // 检查是否已经有任务在下�?  const existingTask = downloadingTasks.get(taskKey);
  if (existingTask) {
    return existingTask;
  }
  ...
  const downloadTask = (async () => {
    try {
      ...
      console.error('[Music Cache] 缓存音频�?OpenList 失败:', error);
    } finally {
...
async function replaceAudioUrlsWithOpenList(
  ...
  for (const song of songs) {
    ...
    try {
      // 只有在未确认缓存状态时才调�?getFile()
      const fileResponse = await openListClient.getFile(audioPath);

      if (fileResponse.code === 200 && fileResponse.data?.raw_url) {
        // 如果启用缓存代理，返回代理URL；否则返回直接URL
        if (cacheProxyEnabled) {
          // 使用代理URL，通过我们的服务器代理OpenList的音�?          song.url = `/api/music/audio-proxy?platform=${platform}&id=${song.id}&quality=${quality}`;
        } else {
...
// 获取方法配置并执行请�?async function executeMethod(
  baseUrl: string,
  platform: string,
  func: string,
  ...
  for (const [key, value] of Object.entries(variables)) {
    // 尝试将字符串转换为数字（如果可能�?    const numValue = Number(value);
  ...
  function processTemplateValue(value: any): any {
    if (typeof value === 'string') {
      ...
      return value.replace(expressionRegex, (match, expression) => {
        try {
          // �?Cloudflare 环境下，使用简单的表达式替�?          if (isCloudflare) {
            const expr = expression.trim();

            // 检查是否是单个变量（没有运算符�?            if (evalContext.hasOwnProperty(expr)) {
              // 直接返回变量�?              return String(evalContext[expr]);
            }

            // 处理包含运算的表达式（如 page - 1�?            let result: any = expr;

            // 替换变量为其�?            for (const [key, val] of Object.entries(evalContext)) {
            ...
            // 尝试计算表达�?            try {
              ...
              console.error(`[executeMethod] Cloudflare 环境执行表达式失�? ${expr}`, err);
          ...
          console.error(`[executeMethod] 执行表达式失�? ${expression}`, err);
          return '0'; // 默认�?        }
      });
  ...
  // 5. 执行 transform 函数（如果有�?  if (config.transform) {
    // �?Cloudflare 环境下，�?transform 函数返回给前端执�?    if (isCloudflare) {
      // �?transform 函数字符串附加到响应数据�?      data.__transform = config.transform;
    } else {
      // �?Node.js 环境下，直接执行 transform
      try {
        // eslint-disable-next-line no-eval
...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (action) {
      case 'toplists': {
        // 获取排行榜列�?        const platform = searchParams.get('platform');
        if (!platform) {
          return NextResponse.json(
      ...
      case 'toplist': {
        // 获取排行榜详�?        const platform = searchParams.get('platform');
        const id = searchParams.get('id');

        if (!platform || !id) {
          return NextResponse.json(
            { error: '缺少 platform �?id 参数' },
            { status: 400 }
          );
        }
      ...
      case 'search': {
        ...
        // 注意：不同平台可能使用不同的变量�?        // 统一传�?keyword, page, pageSize, limit (limit = pageSize)
        const data = await executeMethod(baseUrl, platform, 'search', {
          keyword,
          page,
          pageSize,
          limit: pageSize, // 有些平台使用 limit 而不�?pageSize
        });
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      case 'parse': {
        // 解析歌曲（需�?Token�?        if (!token) {
          return NextResponse.json(
            {
              code: -1,
              error: '未配置音乐服�?Token',
              message: '未配置音乐服�?Token'
            },
            { status: 403 }
          );
        }
        ...
        if (!platform || !ids) {
          return NextResponse.json(
            {
              code: -1,
              error: '缺少 platform �?ids 参数',
              message: '缺少 platform �?ids 参数'
            },
            { status: 400 }
          );
        }
        ...
        // 2. 检查内存缓�?        const cached = serverCache.proxyRequests.get(cacheKey);
        if (cached && Date.now() - cached.timestamp < serverCache.CACHE_DURATION) {
          // 如果启用�?OpenList，需要检查并替换音频 URL
          if (openListClient) {
            ...
            // 没有 OpenList 配置，直接返回内存缓�?            return NextResponse.json(cached.data);
          }
        }

        // 3. 检�?OpenList JSON 缓存
        if (openListClient) {
          try {
        ...
        try {
          ...
          // 6. 检查并替换音频 URL �?OpenList URL（如果已缓存�?          // 同时异步下载未缓存的音频
          ...
          if (openListClient) {
            ...
            openListClient.uploadFile(jsonPath, JSON.stringify(finalData, null, 2))
              .catch((error) => {
                console.error('[Music Cache] 缓存解析结果�?OpenList 失败:', error);
              });
          }
