...
// 全局下载器实�?let downloader: OfflineDownloader | null = null;

// 任务存储（内存中�?const tasks = new Map<string, OfflineDownloadTask>();

// 活跃的下载Promise
const activeDownloads = new Map<string, Promise<void>>();

// 任务持久化文件路�?const TASKS_FILE = path.join(OFFLINE_DOWNLOAD_DIR, 'tasks.json');
...
function loadTasks(): void {
  try {
    ...
    for (const taskData of tasksArray) {
      ...
      // 如果任务在下载或等待中，说明服务器重启了，将状态改为暂�?      if (task.status === 'downloading' || task.status === 'pending') {
        task.status = 'paused';
        task.errorMessage = '服务器重启，任务已暂�?;
      }

      tasks.set(task.id, task);
    }

    console.log(`已加�?${tasks.size} 个离线下载任务到内存`);
  } catch (error) {
...
export async function GET(request: NextRequest) {
  ...
  // 确保下载器已初始化（这会触发任务加载�?  getDownloader();
  ...
  // 获取所有任务列�?  const taskList = Array.from(tasks.values()).map((task) => ({
    ...task,
    // 转换 Date 对象�?ISO 字符�?    createdAt: task.createdAt.toISOString(),
    updatedAt: task.updatedAt.toISOString(),
  }));
...
export async function POST(request: NextRequest) {
  ...
  try {
        ...
        return NextResponse.json(
          {
            ...
            message: '该任务正在下载中，请勿重复添�?,
          },
          { status: 400 }
        );
      }

      // 如果任务已完成，不允许重复创�?      if (existingTask.status === 'completed') {
        return NextResponse.json(
          {
            task: {
              ...existingTask,
    ...
    // 创建新任�?    const task = await downloader.createTask(source, videoId, episodeIndex, title, m3u8Url, metadata);
    tasks.set(task.id, task);
    saveTasks(); // 持久化任�?
    // 开始下载（异步�?    const downloadPromise = downloader
      .startDownload(task, (updatedTask) => {
        // 更新任务状�?        tasks.set(updatedTask.id, updatedTask);
        saveTasks(); // 持久化任�?      })
      .catch((error) => {
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (downloadPromise) {
      // 将任务状态设置为 error，这样下载器会停止下�?      task.status = 'error';
      ...
      // 等待一小段时间，让下载操作有机会停�?      await new Promise(resolve => setTimeout(resolve, 500));
    }
    ...
    tasks.delete(taskId);
    saveTasks(); // 持久化任�?
    return NextResponse.json({ message: '任务已删�? });
  } catch (error) {
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    // 检查任务状态，只有错误、暂停或完成状态可以重�?    if (task.status === 'downloading' || task.status === 'pending') {
      return NextResponse.json({ error: '任务正在进行中，无法重试' }, { status: 400 });
    }

    // 检查是否已经在重试�?    if (activeDownloads.has(taskId)) {
      return NextResponse.json({ error: '任务已在重试�? }, { status: 400 });
    }
    ...
    task.status = 'pending';
    // 不重�?progress �?downloadedSegments，让下载器自动跳过已下载的片�?    task.errorMessage = undefined;
    task.updatedAt = new Date();
    tasks.set(taskId, task);
    saveTasks(); // 持久化任�?
    // 开始重新下载（异步�?    const downloadPromise = downloader
      .startDownload(task, (updatedTask) => {
        // 更新任务状�?        tasks.set(updatedTask.id, updatedTask);
        saveTasks(); // 持久化任�?      })
      .catch((error) => {
