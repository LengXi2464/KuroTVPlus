...
export default function MovieRequestPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setAlertModal({ isOpen: true, type: 'error', title: '搜索失败', message: '请稍后重�? });
    } finally {
  ...
  const submitRequest = async (item: TMDBResult, season?: number) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setAlertModal({ isOpen: true, type: 'error', title: '求片失败', message: data.error || '请稍后重�? });
      }
    } catch (err) {
      console.error('求片失败:', err);
      setAlertModal({ isOpen: true, type: 'error', title: '求片失败', message: '请稍后重�? });
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className='container mx-auto px-4 py-6'>
        <div className='mb-6'>
          ...
          <p className='text-sm text-gray-500 dark:text-gray-400 mt-1'>
            {isFeatureEnabled ? '搜索并提交您想看的影�? : '求片功能已关闭，仅可查看已求片列�?}
          </p>
        </div>
                  ...
                  >
                    {request.poster ? (
                      ...
                      <div className='w-full aspect-[2/3] bg-gray-200 dark:bg-gray-700 flex items-center justify-center'>
                        <span className='text-gray-400'>无海�?/span>
                      </div>
                    )}
                    <div className='p-3'>
                      ...
                      <p className='text-xs text-gray-500 dark:text-gray-400 mb-2'>
                        {request.year || '未知'} · {request.requestCount}人求�?                      </p>
                      <div className={`text-xs px-2 py-1 rounded text-center ${
                        request.status === 'fulfilled'
                          ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300'
                          : 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300'
                      }`}>
                        {request.status === 'fulfilled' ? '已上�? : '待处�?}
                      </div>
                    </div>
                  </div>
                ))}
            ...
            {searchResults.map((item) => (
              ...
              >
                ...
                <div className='p-3'>
                  ...
                  >
                    {submitting ? '处理�?..' : !isFeatureEnabled ? '功能已关�? : '求片'}
                  </button>
                </div>
              </div>
            ))}
