...
// 动态生�?metadata，支持配置更新后的标题变�?export async function generateMetadata(): Promise<Metadata> {
...
export default async function RootLayout({
  ...
  if (storageType !== 'localstorage') {
    ...
    // 自定义去广告代码版本�?    customAdFilterVersion = config.SiteConfig?.CustomAdFilterVersion || 0;
    // 音乐功能配置
    ...
    // 电子书功能配�?    const opdsConfig = config.OPDSConfig;
  ...
  // 将运行时配置注入到全局 window 对象，供客户端在运行时读�?  const runtimeStorageType = process.env.NEXT_PUBLIC_STORAGE_TYPE || 'localstorage';
