...
export default function ScrollableRow({
  ...
  const checkScroll = () => {
    if (containerRef.current) {
      ...
      // 计算是否需要左右滚动按�?      const threshold = 1; // 容差值，避免浮点误差
  ...
  useEffect(() => {
    // 多次延迟检查，确保内容已完全渲�?    checkScroll();

    // 监听窗口大小变化
    window.addEventListener('resize', checkScroll);

    // 创建一�?ResizeObserver 来监听容器大小变�?    const resizeObserver = new ResizeObserver(() => {
      // 延迟执行检�?      checkScroll();
    });
  ...
  }, [children]); // 依赖 children，当子组件变化时重新检�?
  // 添加一个额外的效果来监听子组件的变�?  useEffect(() => {
    if (containerRef.current) {
      // 监听 DOM 变化
