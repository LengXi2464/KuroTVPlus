...
export default function DoubanComments({ doubanId }: DoubanCommentsProps) {
  ...
  const fetchComments = useCallback(async (startIndex: number) => {
    try {
      console.log('正在获取评论，起始位�?', startIndex);
      setLoading(true);
      setError(null);
      ...
      if (startIndex === 0) {
        ...
        setComments((prev) => {
          console.log('追加评论，之�?', prev.length, '新增:', data.comments.length);
      ...
      console.log('更新后状�?- hasMore:', data.hasMore, 'total:', data.total);
    } catch (err) {
  ...
  useEffect(() => {
    // 重置状态当 doubanId 变化�?    setHasStartedLoading(false);
    setComments([]);
    setLoading(false);
    setError(null);
    setTotal(0);
    setHasMore(false);
  }, [doubanId]); // 只在 doubanId 变化时重新获�?
  const startLoading = () => {
    console.log('开始加载评�?);
  ...
  // 如果评论功能被禁用，不显示任何内�?  if (!enableComments) {
    return null;
  }
    ...
    <div className='space-y-4'>
      ...
      {total > 0 && (
        <div className='text-sm text-gray-600 dark:text-gray-400'>
          {total > comments.length ? `�?${total} 条短评` : `已加�?${comments.length} 条短评`}
        </div>
      )}
