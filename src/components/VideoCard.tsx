...
export interface VideoCardProps {
  ...
  isUpcoming?: boolean; // 是否为即将上�?  seasonNumber?: number; // 季度编号
  seasonName?: string; // 季度名称
  orientation?: 'vertical' | 'horizontal'; // 卡片方向
  playTime?: number; // 当前播放时间（秒�?  totalTime?: number; // 总时长（秒）
  cmsData?: {
    desc?: string;
    episodes?: string[];
...
const VideoCard = forwardRef<VideoCardHandle, VideoCardProps>(function VideoCard(
  ...
  const [searchFavorited, setSearchFavorited] = useState<boolean | null>(null); // 搜索结果的收藏状�?  const [showAIChat, setShowAIChat] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchFavoriteStatus = async () => {
      try {
        ...
        throw new Error('检查收藏状态失�?);
      }
    };

    fetchFavoriteStatus();

    // 监听收藏状态更新事�?    const storageKey = generateStorageKey(actualSource, actualId);
    const unsubscribe = subscribeToDataUpdates(
      'favoritesUpdated',
      (newFavorites: Record<string, any>) => {
        // 检查当前项目是否在新的收藏列表�?        const isNowFavorited = !!newFavorites[storageKey];
  ...
  const handleToggleFavorite = useCallback(
    async (e: React.MouseEvent) => {
      ...
      try {
        // 确定当前收藏状�?        const currentFavorited = from === 'search' ? searchFavorited : favorited;
  ...
  const handleClick = useCallback(() => {
    // 即将上映的电影：单击显示上映倒计时提示，不跳�?    if (isUpcoming) {
    ...
    if (origin === 'live' && actualSource && actualId) {
      // 直播内容跳转到直播页�?      const url = `/live?source=${actualSource.replace('live_', '')}&id=${actualId.replace('live_', '')}`;
      router.push(url);
      ...
      if (isCurrentlyOnPlayPage) {
        // �?play 页面内，添加 _reload 参数强制刷新
        url += `&_reload=${Date.now()}`;
        window.location.href = url;
      } else {
        // 不在 play 页面，正常跳�?        router.push(url);
      }
  ...
  // 检查搜索结果的收藏状�?  const checkSearchFavoriteStatus = useCallback(async () => {
  ...
  const handleLongPress = useCallback(() => {
    if (!showMobileActions) { // 防止重复触发
      // 立即显示菜单，避免等待数据加载导致动画卡�?      setShowMobileActions(true);

      // 异步检查收藏状态，不阻塞菜单显�?      if (from === 'search' && !isAggregate && actualSource && actualId && searchFavorited === null) {
  ...
  // 计算距离上映的天数（使用本地时区�?  const daysUntilRelease = useMemo(() => {
    if (!isUpcoming || !releaseDate) return null;

    // 获取今天的本地日期（午夜�?    const today = new Date();
  ...
  const config = useMemo(() => {
    const configs = {
      ...
      douban: {
        ...
        showProgress: false,
        showPlayButton: !isUpcoming, // 即将上映不显示播放按�?        showHeart: false,
        showCheckCircle: false,
        showDoubanLink: false,
        showRating: !!rate,
        showYear: false,
  ...
  // 移动端操作菜单配�?  const mobileActions = useMemo(() => {
    const actions = [];
    ...
    if (config.showHeart && from !== 'douban' && from !== 'tmdb' && actualSource && actualId) {
      ...
      if (from === 'search') {
        ...
        if (searchFavorited !== null) {
          ...
          // 正在加载中，显示占位�?          actions.push({
            id: 'favorite-loading',
            label: '收藏加载�?..',
            icon: <Heart size={20} />,
            onClick: () => { }, // 加载中时不响应点�?            disabled: true,
          });
        }
      } else {
        // 非搜索结果：直接显示收藏选项
    ...
    if (origin !== 'live') {
      actions.push({
        ...
        onClick: () => {
          ...
          // 延迟打开 DetailPanel，确�?MobileActionSheet 完全清理完成
          setTimeout(() => {
    ...
    if (aiEnabled && actualTitle) {
      actions.push({
        ...
        onClick: () => {
          setShowMobileActions(false);
          // 延迟打开 AIChatPanel，确�?MobileActionSheet 完全清理完成
          setTimeout(() => {
            setShowAIChat(true);
          }, 250);
        },
        ...
        >
          ...
          {isDirectPlaySource ? (
            ...
            <Image
              ...
              style={{
                // 禁用图片的默认长按效�?                WebkitUserSelect: 'none',
                userSelect: 'none',
                WebkitTouchCallout: 'none',
                pointerEvents: 'auto', // 改为auto以响应点击事�?                cursor: 'pointer', // 添加指针样式
              } as React.CSSProperties}
              onContextMenu={(e) => {
                e.preventDefault();
                return false;
          ...
          {seasonNumber && (
            <div
              ...
              title={seasonName || `�?{seasonNumber}季`}
            >
              S{seasonNumber}
            </div>
          )}
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {/* 播放源详情悬浮框 */}
                  {(() => {
                    // 优先显示的播放源（常见的主流平台�?                    const prioritySources = ['爱奇�?, '腾讯视频', '优酷', '芒果TV', '哔哩哔哩', 'Netflix', 'Disney+'];

                    // 按优先级排序播放�?                    const sortedSources = uniqueSources.sort((a, b) => {
                    ...
                    const maxDisplayCount = 6; // 最多显�?�?                    const displaySources = sortedSources.slice(0, maxDisplayCount);
                          ...
                          {hasMore && (
                            <div className='mt-1 sm:mt-2 pt-1 sm:pt-1.5 border-t border-gray-700/50'>
                              <div className='flex items-center justify-center text-gray-400'>
                                <span className='text-[10px] sm:text-xs font-medium'>+{remainingCount} 播放�?/span>
                              </div>
                            </div>
                          )}
            ...
            <>
              ...
              >
                ...
                {currentEpisode && actualEpisodes && actualEpisodes > 1 && (
                  ...
                  >
                    第{currentEpisode}�?· 共{actualEpisodes}�?                  </div>
                )}
