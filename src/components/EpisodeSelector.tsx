...
const EpisodeSelector: React.FC<EpisodeSelectorProps> = ({
  ...
  // 标记是否正在进行初始测�?  const [isInitialTesting, setIsInitialTesting] = useState(false);
  const [watchedEpisodes, setWatchedEpisodes] = useState<Set<number>>(new Set());

  // 使用 ref 来避免闭包问�?  const attemptedSourcesRef = useRef<Set<string>>(new Set());
  ...
  // 主要�?tab 状态：'danmaku' | 'episodes' | 'sources'
  // 默认显示选集选项卡，但如果是房员则显示弹�?  const [activeTab, setActiveTab] = useState<'danmaku' | 'episodes' | 'sources'>(
    isRoomMember ? 'danmaku' : 'episodes'
  );

  // 当房员状态变化时，自动切换到弹幕选项�?  useEffect(() => {
  ...
  // 集数过滤设置弹窗状�?  const [showFilterSettings, setShowFilterSettings] = useState<boolean>(false);

  // 读取本地"优选和测�?开关，默认开�?  const [optimizationEnabled] = useState<boolean>(() => {
  ...
  // 读取测速超时设置，默认4�?  const [speedTestTimeout] = useState<number>(() => {
    if (typeof window !== 'undefined') {
  ...
  // 获取视频信息的函�?- 移除 attemptedSources 依赖避免不必要的重新创建
  ...
  // 当有预计算结果时，先合并到videoInfoMap�?  useEffect(() => {
    if (precomputedVideoInfo && precomputedVideoInfo.size > 0) {
      ...
      // 同步更新 ref，确�?getVideoInfo 能立即看到更�?      precomputedVideoInfo.forEach((info, key) => {
        if (!info.hasError) {
  ...
  useEffect(() => {
    const fetchVideoInfosInBatches = async () => {
      if (
        !optimizationEnabled || // 若关闭测速则直接退�?        activeTab !== 'sources' ||
        availableSources.length === 0
      )
        return;

      // 筛选出尚未测速的播放源，并排除不需要测速的源（openlist/emby/xiaoya�?      const pendingSources = availableSources.filter((source) => {
        const sourceKey = `${source.source}-${source.id}`;
        // 跳过已测速的�?        if (attemptedSourcesRef.current.has(sourceKey)) return false;
        // 跳过不需要测速的�?        if (source.source === 'openlist' || source.source === 'emby' || source.source.startsWith('emby_') || source.source === 'xiaoya') return false;
        return true;
      });

      if (pendingSources.length === 0) return;

      // 标记开始初始测�?      setIsInitialTesting(true);
      ...
      // 初始测速完成后，标记为已完�?      setIsInitialTesting(false);
    ...
    // 依赖项保持与之前一�?  }, [activeTab, availableSources, getVideoInfo, optimizationEnabled, initialTestingCompleted, currentSource]);

  // 监听后台加载完成，触发自动测�?  const prevBackgroundLoadingRef = useRef<boolean>(false);
  useEffect(() => {
    // 当后台加载从 true 变为 false 时（即加载完成）
    if (prevBackgroundLoadingRef.current && !backgroundSourcesLoading) {
      // 如果当前选项卡在换源位置，触发测�?      if (activeTab === 'sources' && optimizationEnabled) {
        // 筛选出尚未测速的播放源，并排除不需要测速的源（openlist/emby/xiaoya�?        const pendingSources = availableSources.filter((source) => {
          const sourceKey = `${source.source}-${source.id}`;
          // 跳过已测速的�?          if (attemptedSourcesRef.current.has(sourceKey)) return false;
          // 跳过不需要测速的�?          if (source.source === 'openlist' || source.source === 'emby' || source.source.startsWith('emby_') || source.source === 'xiaoya') return false;
    ...
    // 更新前一次的加载状�?    prevBackgroundLoadingRef.current = backgroundSourcesLoading;
  ...
  // 根据 descending 状态决定分页标签的排序和内�?  const categories = useMemo(() => {
    if (descending) {
      // 倒序时，label 也倒序显示
  ...
  // 添加鼠标悬停状态管�?  const [isCategoryHovered, setIsCategoryHovered] = useState(false);
  ...
  // 处理滚轮事件，实现横向滚�?  const handleWheel = useCallback((e: WheelEvent) => {
  ...
  // 当分页切换时，将激活的分页标签滚动到视口中�?  useEffect(() => {
  ...
  // 处理换源tab点击，只在点击时才搜�?  const handleSourceTabClick = () => {
  ...
  const handleCategoryClick = useCallback(
    (index: number) => {
      if (descending) {
        // 在倒序时，需要将显示索引转换为实际索�?        setCurrentPage(pageCount - 1 - index);
      } else {
  ...
  // 解析网速字符串，转换为 KB/s 数值用于排�?  const parseSpeedToKBps = useCallback((speedStr: string): number => {
    if (!speedStr || speedStr === '未知' || speedStr === '测量�?..') {
      return -1; // 无效速度返回 -1，排在最�?    }
  ...
  // 重新测试单个�?  const handleRetestSource = useCallback(
    async (source: SearchResult, e: React.MouseEvent) => {
      e.stopPropagation(); // 阻止事件冒泡，避免触发换�?      const sourceKey = `${source.source}-${source.id}`;

      // 标记为正在测�?      setRetestingSources((prev) => new Set(prev).add(sourceKey));
      ...
      try {
        ...
        // 无论成功或失败，都移除测试标�?        setRetestingSources((prev) => {
          const newSet = new Set(prev);
          newSet.delete(sourceKey);
          return newSet;
        });
        ...
        <>
          ...
          <div className='flex items-center gap-4 mb-4 border-b border-gray-300 dark:border-gray-700 -mx-6 px-6 flex-shrink-0'>
            ...
            <button
              ...
              onClick={() => {
                // 切换集数排序（正�?倒序�?                setDescending((prev) => !prev);
              }}
            >
              <svg
                className='w-4 h-4'
          ...
          <div className='flex flex-wrap gap-3 overflow-y-auto flex-1 content-start pb-4'>
            {(() => {
              ...
              // 过滤掉被屏蔽的集数，但保持原有索�?              return episodes
                  ...
                  return (
                    ...
                    >
                      ...
                      {(() => {
                        ...
                        // 如果�?OVA 格式，直接返回完整标�?                        if (title.match(/^OVA\s+\d+/i)) {
                          return title;
                        }
                        ...
                        // 如果匹配"第X�?�?第X�?�?X�?�?X�?格式，提取中间的数字（支持小数）
                        const match = title.match(/(?:�??(\d+(?:\.\d+)?)(?:集|�?/);
                        if (match) {
                          return match[1];
                        }
                        return title;
        ...
        <div className='flex flex-col h-full mt-2'>
          ...
          {!sourceSearchLoading && !sourceSearchError && availableSources.length > 0 && (
            <div className='flex justify-end mb-2 px-2 pb-2 border-b border-gray-300 dark:border-gray-700'>
              ...
              >
                {isRetestingAll ? '重测�?..' : isInitialTesting ? '测速中...' : '全部重测'}
              </button>
            </div>
          )}
              ...
              <div className='flex-1 overflow-y-auto space-y-2 pb-20'>
                {availableSources
                  .sort((a, b) => {
                    ...
                    // 当前源始终置�?                    if (aIsCurrent && !bIsCurrent) return -1;
                    if (!aIsCurrent && bIsCurrent) return 1;

                    // 如果初始测速已完成，按网速排序（快的在前�?                    if (initialTestingCompleted) {
                        ...
                        <div className='flex-1 min-w-0 flex flex-col justify-between h-20'>
                            ...
                            {(() => {
                              ...
                              if (videoInfo && videoInfo.quality !== '未知') {
                                if (videoInfo.hasError) {
                                  ...
                                  // 根据分辨率设置不同颜色：2K�?K为紫色，1080p�?20p为绿色，其他为黄�?                                  const isUltraHigh = ['4K', '2K'].includes(
                                    videoInfo.quality
                                  );
                          ...
                          <div className='flex items-end justify-between h-6'>
                            ...
                            {(() => {
                              ...
                              // 只有第一次测试完成后（有测速数据）才显示重新测试按�?                              if (videoInfo) {
                                return (
                                  <button
