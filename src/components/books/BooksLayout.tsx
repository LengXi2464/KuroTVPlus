...
function getStaticMeta(pathname: string) {
  if (pathname === '/books/shelf') return { title: '电子书书�?, subtitle: '集中管理收藏的电子书' };
  if (pathname === '/books/history') return { title: '阅读历史', subtitle: '从上次阅读的位置继续' };
  if (pathname === '/books/search') return { title: '电子书搜�?, subtitle: '按书名与作者搜�? };
  if (pathname === '/books/detail') return { title: '电子书详�?, subtitle: '查看书籍信息与可用格�? };
  if (pathname === '/books/read') return { title: '电子书阅�?, subtitle: '分页阅读', backHref: '/books' };
