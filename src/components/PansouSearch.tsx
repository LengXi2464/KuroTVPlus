...
interface PansouSearchProps {
  keyword: string;
  triggerSearch?: boolean; // 触发搜索的标�?  onError?: (error: string) => void;
}
...
export default function PansouSearch({
      ...
      const updates = await Promise.all(
        runningEntries.map(async ([cloudType, state]) => {
          try {
            ...
            if (!response.ok) {
              throw new Error(data.error || '获取检测进度失�?);
            }
            return {
              cloudType,
              taskId: state.taskId,
            ...
            return {
              ...
              task: {
                ...
                status: 'failed',
                error:
                  error instanceof Error ? error.message : '获取检测进度失�?,
  ...
  // 提取搜索函数，以便在重试时调�?  const searchPansou = useCallback(async () => {
    ...
    try {
      ...
      const errorMsg = err.message || '搜索失败，请检查配�?;
      setError(errorMsg);
      onError?.(errorMsg);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    // triggerSearch 变化时触发搜索（无论�?true 还是 false�?    if (triggerSearch === undefined) {
      return;
    }
  ...
  const handleStartCheck = async (cloudType: string, links: PansouLink[]) => {
    try {
      ...
      if (!platform) {
        throw new Error('当前网盘类型暂不支持有效性检�?);
      }
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || '启动检测失�?);
      }
      setCheckStatesByType((prev) => ({
        ...prev,
        [cloudType]: {
  ...
  const handleCancelCheck = async (cloudType: string) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || '停止检测失�?);
      }
      setCheckStatesByType((prev) => ({
        ...prev,
        [cloudType]: {
  ...
  // 网盘类型选项卡横向拖动滚�?  const handleTypeMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
  ...
  const renderBody = () => {
    ...
    // 过滤显示的网盘类�?    const filteredCloudTypes =
      selectedType === 'all'
        ? cloudTypes
        : cloudTypes.filter((type) => type === selectedType);

    // 计算每种网盘类型的数�?    const typeStats = cloudTypes.map((type) => ({
      type,
              ...
              <div className='flex flex-wrap items-center gap-2'>
                ...
                {cooldownRemainingMs > 0 && (
                  <span className='text-xs text-orange-600 dark:text-orange-400'>
                    冷却�?{Math.ceil(cooldownRemainingMs / 1000)}s
                  </span>
                )}
              </div>
