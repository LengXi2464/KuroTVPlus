...
export default function DanmakuPanel({
  ...
  const initializedRef = useRef(false); // 标记是否已初始化�?  const fileInputRef = useRef<HTMLInputElement>(null);
  ...
  const handleSearch = useCallback(async (keyword: string) => {
    ...
    try {
      ...
      if (response.success && response.animes.length > 0) {
        ...
        setSearchResults([]);
        setSearchError(
          response.errorMessage || '未找到匹配的剧集，请尝试其他关键�?
        );
      }
    } catch (error) {
      console.error('搜索失败:', error);
      setSearchError('搜索失败，请检查弹�?API 服务是否正常运行');
      setSearchResults([]);
    } finally {
  ...
  // 选择动漫，加载剧集列�?  const handleAnimeSelect = useCallback(async (anime: DanmakuAnime) => {
  ...
  const handleEpisodeSelect = useCallback(
    (episode: DanmakuEpisode) => {
      ...
      const selection: DanmakuSelection = {
        ...
        searchKeyword: searchKeyword.trim() || undefined, // 使用当前搜索框的关键�?      };
  ...
  useEffect(() => {
    if (videoTitle && !initializedRef.current) {
      ...
      initializedRef.current = true; // 标记已初始化，防止后续自动填�?    }
  }, [videoTitle]);
  ...
  const getEpisodeDisplayLabel = useCallback((episodeTitle: string, episodeNumber: number) => {
    ...
    const match = episodeTitle.match(/(?:�??(\d+(?:\.\d+)?)(?:集|�?/);
    if (match) {
      return match[1];
    }
          ...
          >
            ...
            {currentSelection.danmakuCount !== undefined && (
              <p className='mt-1 text-xs text-gray-500 dark:text-gray-500'>
                ...
                {currentSelection.danmakuOriginalCount && ` (原始 ${currentSelection.danmakuOriginalCount} �?`}
              </p>
            )}
          </div>
        )}
