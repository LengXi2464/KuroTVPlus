...
const SearchResultFilter: React.FC<SearchResultFilterProps> = ({ categories, values, onChange }) => {
  ...
  const calculateDropdownPosition = (categoryKey: SearchFilterKey) => {
    ...
    if (element) {
      ...
      let x = rect.left;
      // 为标题筛选设置更大的最小宽度，其他保持原来的最小宽�?      const minWidth = categoryKey === 'title' ? 400 : 240;
  ...
  useEffect(() => {
    const handleScroll = () => {
      // 滚动时直接关闭面板，而不是重新计算位�?      if (activeCategory) {
        setActiveCategory(null);
      }
    };
    const handleResize = () => {
      ...
      <div className='relative inline-flex rounded-full p-0.5 sm:p-1 bg-transparent gap-1 sm:gap-2'>
        ...
        <div className='relative'>
          <button
            ...
            aria-label={`按年�?{mergedValues.yearOrder === 'none' ? '排序' : mergedValues.yearOrder === 'desc' ? '降序' : '升序'}排序`}
          >
            <span>年份</span>
